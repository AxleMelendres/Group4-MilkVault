...
function renderMessages(messages, chatPartnerName) {
    ...
    messages.forEach((msg) => {
        // Admin's messages should be on the right (blue)
        const isAdmin = msg.sender_type === 'admin';

        const wrapper = document.createElement('div');
        // Align RIGHT if admin sent it, LEFT if customer sent it
        wrapper.className = `d-flex flex-column mb-2 ${isAdmin ? 'align-items-end' : 'align-items-start'}`;

        const bubble = document.createElement('div');
        // Blue bubble for admin, gray for customer
    ...
    if (chatSubtitle && messages.length > 0) {
        chatSubtitle.textContent = `Last message at ${formatTimestamp(messages[messages.length - 1].created_at)}`;
    }
}
...
    
fetch(`../PHP/adminFetchMessages.php?chat_with=${encodeURIComponent(activeCustomerId)}`, {
    method: 'GET',
...
            throw new Error(`HTTP ${res.status} error from adminFetchMessages.php`);
          });
        }
        return res.json();
      })
